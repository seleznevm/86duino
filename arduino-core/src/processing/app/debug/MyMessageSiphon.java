 ...
 */
public class MyMessageSiphon implements Runnable {
  ...
  public MyMessageSiphon(InputStream stream, MessageConsumer consumer) {
    this(stream, consumer, 0, true);
  }
  
  public MyMessageSiphon(InputStream stream, MessageConsumer consumer, int lineTimeout, boolean stdout) {
    ...
    thread.setName("MyMessageSiphon");
      ...
      while (canRun) {
        ...
        while (lineTimeout == 0 || currentLine.length() == 0 || streamReader.ready()) {
          ...
          if (c == '\n') {
            ...
            consumer.message(currentLine.toString(), this._stdout);
