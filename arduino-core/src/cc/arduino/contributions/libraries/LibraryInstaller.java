...
import processing.app.debug.TargetPlatform; // for 86Duino
...
public class LibraryInstaller {
  ...
  public synchronized void updateIndex(ProgressListener progressListener) throws Exception {
    ...
    TargetPlatform targetPlatform = BaseNoGui.getTargetPlatform(); // for 86Duino
    if (targetPlatform.getPreferences().get("name").indexOf("Vortex86") >= 0) { // for 86Duino
      Constants.LIBRARY_INDEX_URL = "https://raw.githubusercontent.com/acen2009/86Duino_Supports/master/library_86Duino_index.json";
      Constants.LIBRARY_INDEX_URL_GZ = "https://github.com/acen2009/86Duino_Supports/releases/download/Library_86Duino_package/library_86Duino_index.json.gz";
    }                                                                              
    // Create temp files
    ...
    if (targetPlatform.getPreferences().get("name").indexOf("Vortex86") >= 0) {
        ...
        if (downloader.verifyDomain(signatureUrl)) {
          if (downloader.checkSignature(progress, signatureUrl, progressListener, signatureVerifier, statusText, libraryIndexTemp)) {
            ...
            System.out.println(I18n.format(tr("Fail to verify the signature of {0} the cached files have been removed"), libraryURL));
          }
        } else {
          System.out.println(I18n.format(tr("The domain is not selected to verify the signature. library index: {0}"), signatureUrl));
        }
    } 
